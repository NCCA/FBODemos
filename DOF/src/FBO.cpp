...
std::unique_ptr<FBO> FBO::create(int _w, int _h, size_t _numAttatchments)
{
  // note can't use make_unique here as private ctor being invoked.
  return std::unique_ptr<FBO>(new FBO(_w,_h,_numAttatchments));
}

void FBO::setViewport() const
{
  glViewport(0, 0, m_width, m_height);

}
FBO::FBO(int _w, int _h, size_t _numAttatchments) : m_width(_w),m_height(_h)
{
...
bool FBO::addDepthBuffer( GLTextureDepthFormats _format, GLTextureDataType _type, GLTextureMinFilter _min,
...
bool FBO::addColourAttachment(const std::string &_name, GLenum _attachment,
...
GLuint FBO::getTextureID(const std::string &_name)
{
  GLuint id=0;
...
bool FBO::bindToSampler(const std::string &_name,GLuint _location)
{

}
void FBO::print() const
{
  if(m_bound !=true)
  {
    ngl::msg->addError("Trying to print unbound FBO\n");
    return;
  }
  int res;
...
bool FBO::isComplete(Target _target)
{
